...
import csv
import cv2
from flask import Flask, render_template, render_template_string, jsonify, Response, send_file, make_response
...
# ================= WRITING DATA (NEW FOR LIVE METRICS) =================
...
def update_shared_data(count, lat=None, lon=None):
    ...
    # Keep a maximum of 200 items in shared data to keep page lightweight
    if len(data) > 200:
        data = data[-200:]
...
def log_detection(count, lat=None, lon=None):
    ...
    # Keep a maximum of 200 items in logs to keep page lightweight
    if len(logs) > 200:
        logs = logs[-200:]
...
def generate_frames():
    global last_log_time, camera_stream
    ...
    while True:
        ...
        # Resize for smooth processing and transmission
        ...
        status, color_bgr = get_road_status(pothole_count)

        # Draw Futuristic Cyberpunk Target Lock Brackets in OpenCV
        for box in results[0].boxes:
            ...
            # Severity color logic (Green, Orange, Red)
            if pothole_count >= 5:
        ...
        cv2.putText(
            annotated_frame,
            f"POTHOLES: {pothole_count} ({status})",
            (20, 75),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.55,
        ...
        cv2.putText(
            annotated_frame,
            f"TIME: {now_str}",
            (annotated_frame.shape[1] - 170, 40),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.45,
            (255, 255, 255),
            1
...
<div id="system-boot-overlay">
  <div class="boot-container">
    ...
    <div class="boot-terminal" id="boot-terminal-log">
      <div>[OK] INITIALIZING NEURAL ENGINE...</div>
      <div>[OK] LINKING OPENCV CAPTURE DEVICE...</div>
      <div>[OK] LOADING COGNITIVE WEIGHTS FOR YOLOv8...</div>
      <div>[OK] CALIBRATING TACTICAL SURFACE SENSORS...</div>
      <div>[OK] ESTABLISHING GPS SECURE SAT-LINK...</div>
    </div>
  ...
  <div>
    ...
    <div class="cards-grid">
      ...
      <div class="card card-pink card-cyber-corners">
        ...
        <div class="card-value pink count-up" data-target="{{ data|length }}">0</div>
        <div class="card-sub">
          <span style="color:var(--pink);">●</span> Across active session logs
        </div>
      </div>

      <div class="card card-cyber-corners {% if score >= 80 %}card-green{% elif score >= 50 %}card-orange{% else %}card-red{% endif %}">
        <div class="card-top">
          <div class="card-label">Road Quality Index</div>
          <div class="card-icon-badge">🛣</div>
        </div>
        <div class="card-value {% if score >= 80 %}good{% elif score >= 50 %}warn{% else %}danger{% endif %} count-up" data-target="{{ score }}">0</div>
        <div class="card-sub">
          {% if score >= 80 %}
          <span style="color:var(--green);">●</span> Road Condition Optimal
          {% elif score >= 50 %}
      ...
      <div class="card card-red card-cyber-corners">
        ...
        <div class="card-value danger count-up" data-target="{{ data | selectattr('potholes', 'ge', 5) | list | length }}">0</div>
        <div class="card-sub">
      ...
      <div class="card card-cyber-corners" style="background: linear-gradient(135deg, rgba(0, 240, 255, 0.04), rgba(0, 240, 255, 0.01));">
        ...
        <div class="card-value good">94.7%</div>
        <div class="card-sub">
          <span style="color:var(--green);">●</span> Model confidence rating
        </div>
      </div>
  ...
  <div class="two-col">
    ...
    <div>
      ...
      <div class="camera-container">
        ...
        <!-- HUD details -->
        <div class="hud-telemetry top-left">ROAD_SYS // SURV_ACQ</div>
        ...
        <div class="hud-telemetry bottom-left">
          ...
          <span>FEED RATE: <span id="hud-fps-val">--</span> FPS</span>
        </div>
    ...
    <div style="display: flex; flex-direction: column; gap: 24px;">
      ...
      <div class="panel">
        ...
        <div class="health-wrap">
          <div class="health-ring-wrap">
            <svg class="health-ring-svg" width="180" height="180" viewBox="0 0 180 180">
              ...
              <circle class="health-ring-fill" cx="90" cy="90" r="66"
                id="health-ring"
                stroke="{% if score >= 80 %}#00ffaa{% elif score >= 50 %}#ff9c00{% else %}#ff2a5f{% endif %}"
                stroke-dasharray="414.69"
                stroke-dashoffset="{{ 414.69 - (414.69 * score / 100) }}"
              />
            </svg>
            <div class="health-ring-value">
              <div class="health-ring-num {% if score >= 80 %}good{% elif score >= 50 %}warn{% else %}danger{% endif %}"
                   style="color: {% if score >= 80 %}#00ffaa{% elif score >= 50 %}#ff9c00{% else %}#ff2a5f{% endif %}">
                {{ score }}
              </div>
          ...
          <div class="health-status-badge {% if score >= 80 %}good{% elif score >= 50 %}warn{% else %}poor{% endif %}">
            {{ status }} ROAD
          </div>

          <div class="control-btns-wrap">
            <a href="/map" class="map-btn btn-cyan">🌍 SMART ROAD GPS MAP</a>
            <a href="/api/download_report" class="map-btn btn-pink" id="download-report-btn">📥 DOWNLOAD DETECTIONS CSV</a>
          </div>
        </div>
      </div>
...
(function() {
  ...
  const logTimer = setInterval(() => {
    if (logIndex < additionalLogs.length) {
      ...
      line.textContent = additionalLogs[logIndex];
  ...
  const progressTimer = setInterval(() => {
    ...
    if (progress >= 100) {
      ...
      // Complete boot overlay transition
      setTimeout(() => {
...
function updateClock() {
  ...
  if (mainClock) mainClock.textContent = str;
  ...
  fetch('/api/stats')
    ...
    .then(d => {
      ...
      if (d.score >= 80) {
        ...
        if (safetyRec) safetyRec.textContent = "ROAD CONDITION OPTIMAL. MAINTAIN DESIGNATED VEHICLE SPEEDS.";
...
// Hide report button if endpoint unavailable (fail-safe)
fetch('/api/download_report', { method: 'HEAD' })
  .then(res => {
    if (res.status === 404) {
      document.getElementById('download-report-btn').style.opacity = '0.5';
      document.getElementById('download-report-btn').style.cursor = 'not-allowed';
      document.getElementById('download-report-btn').addEventListener('click', (e) => e.preventDefault());
    }
  }).catch(() => {});
...
(function() {
  ...
  if (SpeechRecognition) {
    ...
    recognition.onstart = () => {
      ...
      if (voiceRing) voiceRing.style.borderColor = 'var(--red)';
      if (voiceCenter) voiceCenter.style.backgroundColor = 'var(--red)';
  ...
  function resetVoiceState() {
    ...
    if (voiceRing) voiceRing.style.borderColor = 'var(--cyan)';
    if (voiceCenter) voiceCenter.style.backgroundColor = 'var(--pink)';
  ...
  function speakResponse(text, action, actionData) {
    ...
    // Choose a nice professional sounding robotic voice if available
    const voices = window.speechSynthesis.getVoices();
    const systemVoice = voices.find(v => v.name.includes("Google US English") || v.name.includes("Daniel") || v.name.includes("Samantha") || v.lang.startsWith("en"));
    if (systemVoice) {
    ...
    utterance.onstart = () => {
      ...
      if (voiceRing) voiceRing.style.borderColor = 'var(--green)';
      if (voiceCenter) voiceCenter.style.backgroundColor = 'var(--green)';
      waveBars.forEach(bar => bar.classList.add('active'));
    };
...
def api_detect():
    ...
    status, _ = get_road_status(potholes_detected)
...
def api_log():
    data = load_data()
    return jsonify(data[-80:])  # last 80 records for Leaflet
...
def api_jarvis_voice_query():
    ...
    # 1. Road condition queries
    if any(k in query for k in ["road condition", "road ahead", "danger zone", "how is the road"]):
        if score >= 80:
    ...
    # 3. Location-based queries
    elif any(k in query for k in ["hebbal", "whitefield", "mg road", "indiranagar", "btm layout", "koramangala"]):
        location = None
        ...
        if location:
            action = "map_search"
            action_data = {"location": location.capitalize()}
            # Fetch custom condition if possible, else default
            try:
                from pothole_data import locations
                loc_info = locations.get(location.capitalize(), {})
                potholes = loc_info.get("potholes", 0)
                quality = loc_info.get("condition", loc_info.get("quality", "Moderate"))
                response_text = f"Scanning tactical grid coordinates for {location.upper()}. Found {potholes} potholes. Road threat level is {quality.upper()}."
            except Exception as e:
                response_text = f"Scanning coordinate link for {location.upper()}. Fetching safety metrics from Leaflet."
...
def api_download_report():
    ...
    # Write historical log details
    for row in data:
        potholes = row.get("potholes", 0)
        ...
        confidence = min(85 + potholes * 2, 99)
    ...
    output.headers["Content-Disposition"] = "attachment; filename=roadguard_pothole_report.csv"
...
@app.route("/map")
def map_view():
    return render_template("map_custom.html")
